...
if (!function_exists('makeTextField')) {
    ...
    function makeTextField($type, $label, $name, $value, $explain = "", $maxlen = 40, $size = 25, $disabled = false) {
        $CI = &get_instance();
        $parms = array(
            'type' => $type,
            'label' => $label,
...
if (!function_exists('makeSubmitButton')) {
    ...
    function makeSubmitButton($label, $title, $name, $value, $css_extras = "", $btnStyle) {
        $CI = &get_instance();
        $parms = array(
        ...
        return $CI->parser->parse($btnStyle, $parms, true);
    }
